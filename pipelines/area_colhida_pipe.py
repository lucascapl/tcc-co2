...
def processar_area_colhida(
    caminho_arquivo: str = "bases/area-colhida-1974-2024-ibge.xlsx",
    agrupar_por_regiao: bool = False,
):
    """
    Processa area colhida (IBGE) e retorna:
    - Estado | Ano | area_colhida, quando agrupar_por_regiao=False;
    - Regiao | Ano | area_colhida, quando agrupar_por_regiao=True.
    """
    df = _processar_area_ibge(caminho_arquivo, "area_colhida")

    if agrupar_por_regiao:
        df = agregar_por_regiao_ano(df)
        df = ordenar_regioes(df, coluna_regiao="Regiao", coluna_ano="Ano")
        return salvar_tratado(df, "area-colhida-regiao")

    return salvar_tratado(df, "area-colhida-estado")
