...
def correlacao_spearman_por_grupo(
    df,
    coluna_grupo="Estado",
    coluna_alvo="co2",
    coluna_ano="Ano",
    ...
    for grupo in base[coluna_grupo].dropna().astype(str).unique():
        ...
        for var in variaveis:
            ...
            coef, p = spearmanr(pares[coluna_alvo_analise], pares[var])
            registro.update({
...
def correlacao_spearman_por_estado(df, **kwargs):
    return correlacao_spearman_por_grupo(df, coluna_grupo="Estado", agregar_para_regiao=False, **kwargs)


def correlacao_spearman_por_regiao(df, **kwargs):
    kwargs.pop("coluna_grupo", None)
    kwargs.pop("agregar_para_regiao", None)
    return correlacao_spearman_por_grupo(df, coluna_grupo="Regiao", agregar_para_regiao=True, **kwargs)
